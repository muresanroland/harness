...
//! is the caller's act, taken only while no run holds the lock. A dev build
//! never checks. The check and the download sit behind `Releases`, with the
//! ureq `GitHub` behind it and a fake in tests.
...
pub(crate) const TARGET: &str = "unsupported";
...
/// One check for `version`, the exe at `exe`: Ok(None) is up to date (or a
/// dev build, which never asks), Ok(Some) a greater release downloaded and
/// chmod 755 beside the exe, Err one reason with no temp file left.
pub(crate) fn check(releases: &dyn Releases, version: &str, exe: &Path) -> Checked {
    if version.ends_with("-dev") {
        return Ok(None);
    }
    let tag = releases.latest_tag()?;
    let current = semver(version).ok_or_else(|| format!("unreadable version {version:?}"))?;
...
/// Replaces this process with `exe` and the same argv; returns only on failure.
pub(crate) fn reexec(exe: &Path) -> String {
    let err = Command::new(exe).args(std::env::args_os().skip(1)).exec();
    format!("re-exec refused: {err}")
}

/// init's check, before the gate: a greater release installs and the process
/// re-execs, so init installs skills from the binary it just fetched. A
/// failure is one line, and init goes on.
pub(crate) fn at_init(out: &mut dyn Write) {
    ...
    let err = match checked {
        Ok((Some(ready), exe)) => {
            let _ = writeln!(out, "updating to {}", ready.tag);
            match ready.install() {
                Ok(()) => reexec(&exe),
                Err(err) => err,
            }
...
mod tests {
    ...
    fn a_greater_version_downloads_and_swaps() {
        let dir = TempDir::new();
        let exe = exe(&dir);
        let releases = FakeReleases::new("v1.1.0", b"new");
    ...
    fn an_equal_or_lesser_version_does_not_download() {
        ...
        assert!(check(&FakeReleases::new("v1.10.0", b"new"), "v1.9.0", &exe)
            .unwrap()
            .is_some());
    ...
    fn a_refused_rename_reports_and_drops_the_temp_file() {
        let dir = TempDir::new();
        let exe = exe(&dir);
        let ready = check(&FakeReleases::new("v1.1.0", b"new"), "v1.0.0", &exe)
            .unwrap()
            .unwrap();
    ...
    fn a_dev_build_never_checks() {
        let dir = TempDir::new();
        let exe = exe(&dir);
        let releases = FakeReleases::new("v9.0.0", b"new");
        assert!(check(&releases, "v1.0.0-dev", &exe).unwrap().is_none());
        assert_eq!(
