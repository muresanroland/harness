# harness-7nq.21 — /config: Skills page, Delegate skill per job, TypeSafe page

## Context
/config (src/shell/config.rs + src/shell/draw/config.rs) edits config.json rows only. The left list already shows `Skills N installed` and `TypeSafe on/off`, but they cannot be opened. The skill backend already exists in `src/skills/manifest.rs` (`add`, `update`, `update_all`, `remove`, `list`, `parse_source`, `JOBS`, `Manifest::pick`). It is `#[allow(dead_code)]` "until /config (Ticket 21)". This Ticket wires that backend into /config, adds the Delegate skills to the Stage pages, and adds the TypeSafe page. Every change saves at once. The prototype is `git show prototype/config:src/shell/draw/prototype_config.rs`. The decisions are in harness-0sx.7 and harness-0sx.9.
...
- setup.rs: pull the key write (0600, create .harness) out of `ask_typesafe_key` into `pub(crate) fn keep_key(repo, key) -> io::Result<()>`. Make `KEY_FILE` pub(crate).

### src/shell/config.rs
- Pages: `SKILLS_PAGE = APPS_PAGE + 1` and `TYPESAFE_PAGE = APPS_PAGE + 2`. The left list's Down stops at TYPESAFE_PAGE.
...
- `typing: Option<(Typing, String)>` with `enum Typing { Model(Pick), Source, Key }`.
...
- `confirm: Option<(String, Confirm)>` with `enum Confirm { Remove(String), TypeSafeOff }`. y or Enter says yes; n or Esc says no.
...
- TypeSafe page rows: TypeSafe on/off (on = the key is set and config.json is not off), and the key, masked as ••••+last 4 or none.
  - Enter while on asks "Turn TypeSafe off? Every Wake and Plan becomes a Question; a Finding the Debate still disputes is skipped and listed in the PR." Yes calls `app::set_typesafe(false)`.
  - Enter while off with a key: on at once.
  - Enter while off without a key (on either row): type the key, masked. Enter calls `keep_key`, then `set_typesafe(true)`, then sets `cfg.api_key`. An empty key changes nothing.
...
- The pipeline's Skills and TypeSafe rows get the cursor. Their summaries come from `manifest` and the TypeSafe state.
...
- `typesafe_page`, the checklist (`[x]` / `[ ]`), and the job pick list (title `<Section> <job> · <app>`, a wider name column, colored marks).
- Foot: the busy spinner in orange; the confirmation with `y/n`; the source prompt; the key prompt shown as `•`; the row notes, including the location's "Run harness init again to change where skills are installed."
...
6. TypeSafe off asks first, then saves `"typesafe": false`.
7. On with no key (`cfg.api_key` empty) asks the key. The foot shows only `•`. Enter writes .harness/typesafe-key and saves on.
8. TestBackend renders of the Skills page and of a job's pick list, with exact lines.
...
- Turning TypeSafe on from /config does not install the typesafe-ai skill; init does that. A key typed during a live run reaches the next run, because each run's Config is a clone. This gets a ponytail comment.
- The key row shows the key masked and cannot be edited. A key is asked for only when there is none, which also keeps TYPESAFE_API_KEY in the environment winning.
