# harness-7nq.6 — Epic summary pager, /summary
...
`/summary [@epic]` rebuilds it on demand from bd, the State and the Run directory evidence.
...
- `poll()`: State snapshot moved after the scheduler join (so a finished run's snapshot is final). Then, for an Epic run not yet `summarized`: when every Ticket of the Epic on the bd cache is closed in bd or has State `pr-open`/`merged`/`parked`, at least one with its PR → `summarized = true`, `summarize`. Epic-done branch: remember `last_epic`.
