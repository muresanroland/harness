# harness-7nq.1: five small fixes

## Context
Ticket harness-7nq.1 (first of Epic harness-7nq). Five small changes settled on map harness-0sx / prototype harness-0sx.9 / research harness-0sx.13. Scope = ticket only.
...
3. **BORDER** `src/shell/logo.rs:16`: `Rgb(35, 49, 74)` → `Rgb(84, 98, 124)` (prototype/config's DIVIDER).
4. **Placeholder color** `src/shell/logo.rs`: add `pub(crate) const DARK_ORANGE: Color = Color::Rgb(200, 110, 20);` (name from prototype/screen-updates). `src/shell/draw.rs:519` hint `.fg(BORDER)` → `.fg(DARK_ORANGE)`; import it.
   - Decision: COMPOSING hint shares that one expression, so it goes dark orange too (same slot, prototype colors the hint whatever its text). Placeholder text untouched (Ticket 4).
5. **trust.rs header** `src/orchestrator/trust.rs:1-6`: drop "Codex's does not: reads as idle, result file missing". New: both dialogs register as blocked pane (herdr's codex manifest reads Codex trust screen as blocked); neither answerable by Orchestrator, so it reads trust stores and waits. Grep found no other comment repeating the claim (stage.rs:753 is about any dialog with `--wait`, not Codex; left).
...
- `src/shell/shell_test.rs`
  ...
  - New test `boxes_draw_in_border_and_the_placeholder_in_dark_orange`: `render(&screen(), 120, 40)`; cell left of " RECENT " title (box corner) fg == BORDER; first cell of "/start-epic" fg == DARK_ORANGE; assert `BORDER == Rgb(84,98,124)` and `DARK_ORANGE == Rgb(200,110,20)` (pins values, same style as existing `ticket_color(..) == CYAN`).
