...
- The CLI is `harness` and `harness init`. start, status, stop, retry, park and address become slash commands in the Shell; `harness init` stays a command because it is a stdin prompt, not a run.
